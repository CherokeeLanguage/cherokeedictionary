...
//        }
    ...
    def categorySearch(params) {
        ...
        def lst = Likespreadsheets.findAll('from Likespreadsheets l where l.category = ?', [category.category])
    ...
    def xrefSearch(term) {
        ...
        if (searchTerm.size() == 1)
            return Likespreadsheets.findAll("from Likespreadsheets l where l.entrya like ?", ["%$term%"])
        else {
            List lst = []

            searchTerm.each {
                def leest = Likespreadsheets.findAll("from Likespreadsheets l where l.entrya like ?", ["%$it%"])
                lst.addAll(leest)
            }
